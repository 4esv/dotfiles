...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	// Slow down the animation significantly for background use
	float time = iTime * 0.005;  // Very slow for calm background effect

	float slow = time*0.002;
	effectUv *= 1. + .5*slow*sin(slow*10.);

	float ts = time*0.37;
	float change = ts;	// Smooth continuous change instead of discrete flicks

	vec3 p = vec3(effectUv*.2,slow+change);					//coordinate + slight change over time
...
//	colorVec = colorVec / (1. + length(colorVec));	//tone it all down a bit
	...
	// Terminal integration - blend with terminal content
	vec4 terminalColor = texture(iChannel0, uv);
	float textPresence = length(terminalColor.rgb);
	float effectStrength = 0.15;  // Subtle background effect
	float blendFactor = effectStrength * (1.0 - textPresence * 0.85);

	fragColor = vec4(mix(terminalColor.rgb, colorVec, blendFactor), terminalColor.a);
}
