...
//  Modified for Ghostty terminal compatibility
//  Original: Now with music. No more silent silicon dreaming.
//  Original music: OurPithyAtor by In-FaZe
...
// Note: Original shader used audio input (iChannel0) for beat detection
// In ghostty, we simulate this with time-based patterns
float beat() {
  // Simulate beat without audio input using time-based pulses
  return 0.5 + 0.5 * sin(iTime * 2.0);
}

float freq(float x) {
  // Simulate frequency response without audio input
  return 0.3 + 0.2 * sin(x * 10.0 + iTime);
}
...
void mainImage(out vec4 O, vec2 C) {
  ...
  // Render the background effect with flipped Y coordinate
  vec2 renderCoord = (C+C-r)/r.y;
  renderCoord.y = -renderCoord.y;  // Flip Y for correct orientation
  vec4 effect = render(renderCoord, uv);

  // Sample the terminal texture (no flip needed here)
  vec4 terminalColor = texture(iChannel0, uv);

  // Blend the effect with terminal content
  // Make terminal text more visible by reducing effect where text is present
  float textPresence = length(terminalColor.rgb);
  float blendFactor = 0.3 * (1.0 - textPresence * 0.8);

  O = vec4(mix(terminalColor.rgb, effect.rgb, blendFactor), terminalColor.a);
}
