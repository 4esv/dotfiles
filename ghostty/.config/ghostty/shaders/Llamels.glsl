//llamels by eiffie (Z joints), adapted for terminal use
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec3 dr = vec3((2.0*flippedCoord.xy-size.xy)/size.y,2.0);
    ...
    float t = DE(ro)*(0.5+0.5*rnd(flippedCoord.xy)), d, dm = 10.0, tm;
    ...
    // Terminal integration with smart blending
    float textPresence = length(terminalColor.rgb);
    float effectStrength = 0.7;
    float blendFactor = effectStrength * (1.0 - textPresence * 0.8);
    
    // Final output
    fragColor = vec4(mix(terminalColor.rgb, col, blendFactor), terminalColor.a);
}
